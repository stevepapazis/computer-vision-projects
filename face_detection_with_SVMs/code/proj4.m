...
t = toc;
fprintf("%d positive features computed in %f seconds\n", num_pos, t);

tic;
num_negative_examples = 70000; %Higher will work strictly better, but you should start with 10000 for debugging
features_neg = get_random_negative_features( non_face_scn_path, feature_params, num_negative_examples);
num_neg = size(features_neg, 1);
t = toc;
fprintf("%d random negative features computed in %f seconds\n\n", num_neg, t);

labels = zeros( num_pos + num_neg, 1 );
labels(1:num_pos) = 1;
labels(num_pos+1:end) = -1;
...
tic;
[w,b] = vl_svmtrain([features_pos; features_neg].', labels, 0.001);
t = toc;
fprintf('The SVM got trained in %f seconds\n\n', t);
...
clear features_pos features_neg labels;
...
title('Seperation of the positive and negative training examples');
hold off;
clear confidences non_face_confs face_confs label_vector;
...
imhog = vl_hog('render', single(reshape(w, [n_hog_cells n_hog_cells 31]))) ;
